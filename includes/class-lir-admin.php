...
class LIR_Admin {
	...
	public static function render() {
		...
		<div class="wrap" dir="rtl" style="max-width:820px">
			...
			<p>כדי שהמיפוי האוטומטי יעבוד, יש למלא <strong>תגית אינסטגרם</strong> (<code>ig_tag</code>) בכל רופא ובכל טיפול רלוונטי (בלי <code>#</code>). לדוגמה: אצל ד"ר שמואל לוינגר → <code>shmuel_levinger</code>, אצל פרופ' אליה לוינגר → <code>elia_levinger</code>, בטיפול SMILE → <code>smile</code>. בפוסט באינסטגרם משתמשים ב-<code>#shmuel_levinger #smile</code>.</p>
			<p><strong>שימו לב:</strong> התגית מזהה <strong>רופא מסוים</strong>, לא את המרפאה — ולכן יש להשתמש בשם פרטי + שם משפחה (יש יותר מרופא אחד בשם לוינגר).</p>
			<p><strong>כל תגית חייבת להיות ייחודית.</strong> אם שני רופאים — או שני טיפולים — חולקים אותה תגית, המיפוי יבחר רק אחד מהם, ללא הודעת שגיאה. לכן אין להגדיר <code>cataract</code> על כל סוגי הקטרקט: יש להגדיר אותה על הטיפול הכללי בלבד, ולתת לכל וריאציה תגית משלה (<code>laser_cataract</code> וכדומה). כדי לשייך המלצה לכמה טיפולים, מוסיפים כמה תגיות שונות בכיתוב — לא אותה תגית על כמה טיפולים. אין להשתמש באותה תגית גם ברופא וגם בטיפול.</p>
			<p>בכל המלצה נבחר <strong>רופא אחד בלבד</strong> — התגית הראשונה בכיתוב שמתאימה לרופא. תגיות טיפולים נאספות כולן, כך שהמלצה אחת יכולה לכלול כמה טיפולים.</p>
		</div>
		<?php
	}
}
