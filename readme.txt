...
* The Instagram caption is shown under the video, clamped to two lines with an
  "עוד" / "פחות" expander, the way Instagram presents it. It was stored in
  post_content all along but never sent to the front end.
* Hashtags are stripped from the displayed caption: the marker is internal and the
  doctor/procedure tags already appear as chips above it. Windows line endings are
  normalised so the clamp counts real lines.

= 0.5.1 =
* `review_duration` is filled automatically on ingest. Instagram sends no duration, but
  media_handle_sideload() has already run getID3 over the file, so the running time is
  read from the attachment metadata. A duration posted explicitly still wins.

= 0.5.0 =
* Removed the decorative like / save / share buttons from the lightbox. They toggled a
  CSS class and nothing else — no storage, no server call — and reset on every video
  change, so they promised an action the site never performed.
* The procedure chip now links to that procedure's page and the doctor chip (avatar and
  name) to the doctor's page. An `<a>` with no href is left inert when a post has no URL.

= 0.4.0 =
* Filter bar redesigned: procedure filters and the doctor selector are one family of
  text pills with per-filter counts, replacing the icon circles with clamped labels.
  A results line under the bar updates live as filters change. Pills scroll as one
  row on mobile.
* Ingested reviews get "⚠ למלא…" placeholders for name and quote instead of caption
  fragments, so an unfinished draft cannot be published by accident.
